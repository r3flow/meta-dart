...
DART2_RUNTIME ??= "release"

DEPENDS += " \
    ca-certificates-native \
    dart2-bin-sdk-native \
    "
...
DART2_APP_BIN_NAME ??= "${PUBSPEC_APPNAME}.dill"
...
do_archive_pub_cache[depends] += " \
    dart2-bin-sdk-native:do_populate_sysroot \
...
do_compile() {
    ...
    bbnote "dart2 build ${DART2_BUILD_ARGS}: Starting"
    dart compile kernel ${DART2_BUILD_ARGS} -o ${DART2_APP_OUT_DIR}/${DART2_APP_BIN_NAME} ${DART2_ENTRY_POINT}
    bbnote "dart2 build ${DART2_ENTRY_POINT}: Completed"
}
...
do_restore_pub_cache[depends] += " \
    ...
    "
do_install() {
    export DART2_SDK_DIR
    export DART2_APP_INSTALL_DIR
    export DART2_APP_BIN_NAME
    export RUN_SCRIPT_NAME="$(echo ${DART2_APP_BIN_NAME} | sed 's/\(.*\)\..*/\1/')"

    install -d ${D}${DART2_APP_INSTALL_DIR}
    cp ${DART2_APP_OUT_DIR}/${DART2_APP_BIN_NAME} ${D}${DART2_APP_INSTALL_DIR}/${DART2_APP_BIN_NAME}
    echo -e "#!/bin/bash\ncd ${DART2_APP_INSTALL_DIR}\n${DART2_SDK_DIR}/bin/dart run ${DART2_APP_BIN_NAME}\n" > ${D}${DART2_APP_INSTALL_DIR}/${RUN_SCRIPT_NAME}
    chmod +x ${D}${DART2_APP_INSTALL_DIR}/${RUN_SCRIPT_NAME}
}
