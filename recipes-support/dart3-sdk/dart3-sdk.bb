...
do_unpack() {
    ...
    bbnote "dart3-sdk unpack for ${TARGET_ARCH} to ${MACHINE_ARCH}: starting (native)"
    bbnote "dart3-sdk fetch: starting"
    cd ${WORKDIR}/src
    fetch --force dart

    bbnote "dart3-sdk checkout tags/${DART3_SDK_VERSION}: starting"
    cd ${WORKDIR}/src/sdk
    git checkout tags/${DART3_SDK_VERSION}
    gclient sync -D
}
...
do_compile:class-native () {
    ...
    cd ${S}
    bbnote "dart3-sdk x64 compile for ${TARGET_ARCH} to ${MACHINE_ARCH}: starting (native)"
    ${S}/tools/build.py --no-goma --mode release --arch x64 create_sdk
    if $IS_TARGET_ARM64; then
        bbnote "dart3-sdk simarm64 gen_snapshot compile: starting (native)"
        ${S}/tools/build.py --no-goma --mode release -a simarm64 -m product copy_gen_snapshot
        bbnote "dart3-sdk arm64 dartaotruntime compile: starting (native)"
...
do_compile:class-target () {
    ...
    cd ${S}
    bbnote "dart3-sdk ${DART_SDK_TARGET_ARCH} compile: starting (target)"
    ${S}/tools/build.py --no-goma --mode release --arch ${DART_SDK_TARGET_ARCH} create_sdk
}

SYSROOT_PREPROCESS_FUNCS:class-native += "dart3_compiled_sdk_sysroot_preprocess"

dart3_compiled_sdk_sysroot_preprocess () {
   install -d ${SYSROOT_DESTDIR}/${STAGING_DIR_NATIVE}/${DART3_SDK_DIR}
   cp -R ${D}/${DART3_SDK_DIR}/* ${SYSROOT_DESTDIR}/${STAGING_DIR_NATIVE}/${DART3_SDK_DIR}
}

do_install:class-native () {
    install -d ${D}${DART3_SDK_DIR}
    cp -R ${S}/out/ReleaseX64/dart-sdk/* ${D}${DART3_SDK_DIR}
}

do_install:class-target () {
    install -d ${D}${DART3_SDK_DIR}
    cp -R ${S}/out/${DART_SDK_RELEASE_DIR}/dart-sdk/* ${D}${DART3_SDK_DIR}
}

ALLOW_EMPTY:${PN} = "1"

FILES:${PN} += "${DART3_SDK_DIR}/*"
